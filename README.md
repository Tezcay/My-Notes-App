...
项目界面深度复刻了 **「小米笔记」** 的清爽风格，并在此基础上实现了 **响应式布局、夜间模式、拖拽归档、Markdown 编辑** 等高级交互功能，旨在提供极致的 Web 笔记体验。
...
* **🔍 智能搜索高亮**：
    * [cite_start]全文检索笔记标题与内容，匹配关键词自动**高亮显示**（绿色加粗），快速定位 [cite: 40]。

### 🛠 2.2 核心交互

* **📝 笔记管理**：
    * [cite_start]支持新建、编辑（实时保存）、删除笔记 [cite: 37-39]。
    * [cite_start]**Markdown 支持**：集成 EasyMDE，支持加粗、列表、代码块及**图片粘贴上传** [cite: 183-184]。
    * [cite_start]**回收站机制**：普通删除移入“最近删除”，在回收站中再次删除则永久销毁 [cite: 153-154]。
* **📂 文件夹系统**：
    * [cite_start]支持新建自定义文件夹，右键可删除或重命名文件夹（级联逻辑：文件夹删除后，其下笔记自动归入“未分类”）[cite: 35]。
* **🖱️ 拖拽归档 (Drag & Drop)**：
    * [cite_start]支持直接将笔记从列表中**拖拽**到左侧文件夹进行快速分类 [cite: 177-178]。
* **🔃 多维排序**：
    * [cite_start]支持按“编辑时间倒序”、“编辑时间正序”、“标题 A-Z”三种模式切换 [cite: 168]。
* **🔐 私密笔记**：
    * [cite_start]独立的私密笔记入口，支持首次设置密码及后续访问验证，保护敏感内容 [cite: 148-151]。
...
* [ ] **数据备份与恢复**：支持导出为 JSON/Markdown 文件 。
...
| `switchCategory(id, name)` | D1 | 切换当前选中的分类，刷新 UI 并清空编辑器 | `id`: 分类ID, `name`: 分类名 |
...
* **Q: 数据存在哪里？**
* A: 存在浏览器的 LocalStorage 中。清除浏览器缓存会丢失数据，建议手动备份。
